...
typedef struct
{
    bool active;            /* 刷新任务是否进行中 */
    bool full_refresh;      /* true表示全屏刷新 */
    uint8_t page_start;     /* 起始页 */
    uint8_t page_end;       /* 结束页 */
    uint8_t next_page;      /* 下一个待发送的页 */
    uint8_t x_start;        /* 刷新区域起始X */
    uint8_t x_end;          /* 刷新区域结束X */
    uint8_t col_start;      /* OLED寄存器的列起始值 */
    uint8_t col_end;        /* OLED寄存器的列结束值 */
    uint16_t data_len;      /* 每页需要传输的数据长度 */
} OLED_RefreshJob_t;
...
#ifndef OLED_USE_I2C_DMA
#define OLED_USE_I2C_DMA       1    /* 1: 使用I2C DMA传输, 0: 使用阻塞方式传输 */
#endif
