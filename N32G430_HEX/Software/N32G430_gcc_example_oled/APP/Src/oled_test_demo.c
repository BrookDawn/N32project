...
static uint32_t demo_cycle_start_us = 0;           /* Demo循环开始时间（微秒） */
static uint32_t demo_cycle_time_ms = 0;            /* 上一次完整Demo循环耗时（毫秒） */
...
static HAL_StatusTypeDef OLED_TestDemo_PrepareI2CDMA(void)
{
    ...
    if (hi2c1.State == HAL_I2C_STATE_RESET ||
        hi2c1.hdmatx == NULL ||
        hi2c1.hdmatx->State == HAL_DMA_STATE_RESET)
    {
        need_i2c_init = true;
    }
    else if (hi2c1.State == HAL_I2C_STATE_ERROR)
    {
...
void OLED_TestDemo_Init(void)
{
    ...
    demo_cycle_start_us = GetMicrosecond();
}
...
static void OLED_TestDemo_Update(void)
{
    ...
    if (delta_ms >= ANIM_UPDATE_INTERVAL)
    {
        ...
        /* 刷新显示：DMA异步刷屏，CPU与DMA并行 */
#if OLED_USE_I2C_DMA
        if (!OLED_IsRefreshBusy(&holed))
        {
            OLED_RefreshDirtyAsync(&holed);
        }
#else
...
static void OLED_TestDemo_SwitchScene(void)
{
    ...
    /* 如果回到起始场景，记录整个Demo循环耗时 */
    if (current_scene == DEMO_SCENE_START)
    {
        uint32_t now_us = GetMicrosecond();
        uint32_t elapsed_us = (now_us >= demo_cycle_start_us) ?
                              (now_us - demo_cycle_start_us) :
                              (0xFFFFFFFF - demo_cycle_start_us + now_us + 1);
        demo_cycle_time_ms = elapsed_us / 1000;
        demo_cycle_start_us = now_us;
    }
...
/**
  * @brief  在屏幕右上角绘制完整Demo循环耗时
  */
static void DrawLoopTime(void)
{
    char time_str[24];

    if (demo_cycle_time_ms == 0)
    {
        snprintf(time_str, sizeof(time_str), "Loop ...");
    }
    else
    {
        uint32_t secs = demo_cycle_time_ms / 1000;
        uint32_t ms_rem = demo_cycle_time_ms % 1000;
        snprintf(time_str, sizeof(time_str), "Loop %lu.%03lus", secs, ms_rem);
    }
