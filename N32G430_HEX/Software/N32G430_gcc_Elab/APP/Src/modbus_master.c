...
void ModbusMaster_Init(uint8_t slaveAddress)
{
    ...
    ELOG_INFO("[Modbus] Master ready. Slave: 0x%02X, Range: 0x%04X-0x%04X",
...
void ModbusMaster_Task(void)
{
    ...
    if(ModbusMaster_ReadHoldingRegisters(start, quantity, &s_registerCache[start - MODBUS_REGISTER_START]))
    {
        ...
        ELOG_INFO("[Modbus] Read OK: 0x%04X-0x%04X (%u regs)", start, endAddr, quantity);

        s_nextRegister = (endAddr >= MODBUS_REGISTER_END) ? MODBUS_REGISTER_START : (uint16_t)(endAddr + 1U);
    }
    else
    {
        ELOG_ERROR("[Modbus] Read failed at 0x%04X (%u regs)", start, quantity);
    }
...
static bool ModbusMaster_ReadHoldingRegisters(uint16_t startReg, uint16_t quantity, uint16_t* dest)
{
    ...
    if(rxFrame[1] == (MODBUS_FUNC_READ_HOLDING_REGISTERS | MODBUS_EXCEPTION_MASK))
    {
        ...
        if(crcCalc == crcResp)
        {
            ELOG_WARN("[Modbus] Exception code: 0x%02X", exception[0]);
        }
    ...
    if(crcCalc != crcResp)
    {
        ELOG_ERROR("[Modbus] CRC error. Calc:0x%04X Resp:0x%04X", crcCalc, crcResp);
        return false;
    }
...
static bool ModbusMaster_ExecuteWriteSingle(uint16_t address, uint16_t value)
{
    ...
    {
        ELOG_ERROR("[Modbus] Write single timeout");
        return false;
    }
    ...
    {
        ELOG_WARN("[Modbus] Write single exception: 0x%02X", rxFrame[2]);
        return false;
    }
    ...
    if(crcCalc != crcResp)
    {
        ELOG_ERROR("[Modbus] Write single CRC error");
        return false;
    }
    ...
    {
        ELOG_ERROR("[Modbus] Write single echo mismatch");
        return false;
    }

    ELOG_INFO("[Modbus] Write OK: 0x%04X = 0x%04X", address, value);
    return true;
}
...
static bool ModbusMaster_ExecuteWriteMultiple(uint16_t address, uint16_t count, const uint16_t* values)
{
    ...
    if(!ModbusMaster_ReadBytes(rxFrame, 8U))
    {
        ELOG_ERROR("[Modbus] Write multiple timeout");
        return false;
    }
    ...
    {
        ELOG_WARN("[Modbus] Write multiple exception: 0x%02X", rxFrame[2]);
        return false;
    }
    ...
    if(crcCalc != crcResp)
    {
        ELOG_ERROR("[Modbus] Write multiple CRC error");
        return false;
    }
    ...
    {
        ELOG_ERROR("[Modbus] Write multiple echo mismatch");
        return false;
    }

    ELOG_INFO("[Modbus] Write OK: 0x%04X (%u regs)", address, count);
    return true;
}
...
bool ModbusMaster_WriteSingleRegister(uint16_t address, uint16_t value)
{
    ...
    {
        ELOG_WARN("[Modbus] Write queue full");
        return false;
    }
    ...
    ELOG_DEBUG("[Modbus] Queued write single: 0x%04X = 0x%04X", address, value);
    return true;
}
...
bool ModbusMaster_WriteMultipleRegisters(uint16_t address, uint16_t count, const uint16_t* values)
{
    ...
    ELOG_DEBUG("[Modbus] Queued write multiple: 0x%04X (%u regs)", address, count);
    return true;
}
