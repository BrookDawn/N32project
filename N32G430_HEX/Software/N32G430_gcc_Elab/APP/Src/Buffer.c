...
void RingBuffer_Init(Buffer_t *buffer)
{
    ...
    ELOG_DEBUG("RingBuffer_Init: buffer->p_head = %p, buffer->p_tail = %p", buffer->p_head, buffer->p_tail);
#endif
}
...
Buffer_State_t RingBuffer_WriteArray(Buffer_t *buffer, uint8_t *data, uint16_t length)
{
    ...
    {
#if __BUFFER_DEBUG__ == 1
        ELOG_ERROR("RingBuffer_WriteArray: InitError");
#endif
    ...
    {
#if __BUFFER_DEBUG__ == 1
        ELOG_ERROR("RingBuffer_WriteArray: Invalid parameters");
#endif
        return BUFFER_InitError;
    }
...
#if __BUFFER_DEBUG__ == 1
            ELOG_WARN("RingBuffer_WriteArray: Buffer full, moving head by %d bytes", head_move);
#endif
        }
...
#if __BUFFER_DEBUG__ == 1
        ELOG_DEBUG("RingBuffer_WriteArray: Copied %d bytes (offset %d), p_head = %p, p_tail = %p",
...
Buffer_State_t RingBuffer_ReadArray(Buffer_t *buffer, uint8_t *data, uint16_t length)
{
    ...
    {
#if __BUFFER_DEBUG__ == 1
        ELOG_ERROR("RingBuffer_ReadArray: buffer->p_head = %p, buffer->p_tail = %p", buffer->p_head, buffer->p_tail);
        ELOG_ERROR("RingBuffer_ReadArray: InitError");
#endif
        return BUFFER_InitError;
    }
    ...
    {
#if __BUFFER_DEBUG__ == 1
        ELOG_ERROR("RingBuffer_ReadArray: Invalid parameters");
#endif
        return BUFFER_InitError;
    }
...
#if __BUFFER_DEBUG__ == 1
ELOG_DEBUG("RingBuffer_ReadArray: Read data[%d] = %d, p_head = %p, p_tail = %p",
 ...
 uint8_t Data_Read(Buffer_t *buffer, uint8_t *data)
 {
     ...
     {
#if __BUFFER_DEBUG__ == 1
         ELOG_ERROR("Data_Read: Invalid parameters");
#endif
         return 0;
     }
     ...
     {
#if __BUFFER_DEBUG__ == 1
         ELOG_DEBUG("Data_Read: No valid frame found");
#endif
         return 0;
     }
...
#if __BUFFER_DEBUG__ == 1
     ELOG_DEBUG("Data_Read: Read frame data, length = %d", frame_length);
#endif
...
Buffer_State_t Data_Check(Buffer_t *buffer)
{
    ...
    while(search_count < BUFFER_SIZE)/* 循环检测数据，最多搜索整个缓冲区 */
    {
...
#if __BUFFER_DEBUG__ == 1
        ELOG_DEBUG("Data_Check: Found frame header at position %d",
...
#if __BUFFER_DEBUG__ == 1
        ELOG_DEBUG("Data_Check: Frame length = %d", data_length);
#endif
...
#if __BUFFER_DEBUG__ == 1
        ELOG_DEBUG("Data_Check: Calculated checksum = 0x%02X, Stored checksum = 0x%02X",
               temp_checksum & 0xFF, checksum);
        ELOG_DEBUG("Data_Check: Frame tail = 0x%02X", frame_tail);
#endif
        ...
        /* 所有检查都通过，找到有效帧 */
#if __BUFFER_DEBUG__ == 1
        ELOG_DEBUG("Data_Check: Valid frame found, length = %d", data_length);
#endif
        return BUFFER_OK;
    }
